...
-- Tengo que separar los bloques en encoded data y engineer data
-- 1. PRIMER BLOQUE: Transformaciones (Aquí nacen los números)
encoded_data AS (
    SELECT 
        ...
        * EXCEPT(
        ...
        -- [ NUEVO FEATURE ENGINEERING: Insights del Árbol de Residuos ] ----------------

        -- 1. Bandera de sótano masivo inacabado (Umbral lógico: > 1500 sqft)
        --CASE WHEN bsmt_unf_sf > 1500 THEN 1 ELSE 0 END AS is_massive_unfinished_bsmt,

        -- 2. Índice de Deterioro Continuo (De 0 a 10, mayor puntaje = peor estado)
        -- Asumiendo que 5 es 'Ex' y 1 es 'Po'. Restamos a 5 para invertir la escala.
        --((5 - CAST(exter_cond AS INT64)) + (5 - CAST(heating_qc AS INT64))) AS deterioration_index,

        -- 3. Bandera Crítica de Reparación Urgente (Basada directamente en el nodo de error)
        --CASE WHEN CAST(exter_cond AS INT64) <= 2 AND CAST(heating_qc AS INT64) <= 3 THEN 1 ELSE 0 END AS needs_urgent_repair

    FROM intermediate_train
),

-- 2. SEGUNDO BLOQUE: Feature Engineering (Aquí hacemos la matemática)
...
-- filtramos usando los nombres de las variables ya calculadas
SELECT *
FROM engineered_data
WHERE NOT (
    -- poda quirúrgica de residual profiling 1.0
    sale_condition != 'Normal' 
    AND lot_coverage_pct <= 0.068     
)
-- poda quirúrgica de residual profiling 2.0
AND NOT (
    -- Poda del Deck Gigante (1 muestra)
    sale_condition != 'Normal' 
    AND total_outdoor_deck > 697.0
)
AND NOT (
    -- poda: Las 2 casas "Flippeadas" incalculables
    house_age > 60 
    AND years_since_remodel = 0
)